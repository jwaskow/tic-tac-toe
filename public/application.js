...
/***/ function(module, exports, __webpack_require__) {
	...
	var board = ['', '', '', '', '', '', '', '', '']; // board is an empty grid/array

	var turnCount = 0; // tracks the number of turns

	var gameOver = false; // variable that tracks if the game has been won

	var xWins = false; // variable that tracks if x wins

	var oWins = false; // variable that tracks if o wins

	var noneWins = false; // variable that tracks if the game is a draw
	...
	var makeMove = function makeMove(num) {
	  if (gameOver === true) {
	  ...
	  var moveValue = board[num];
	  gameApi.update(num, moveValue).then(gameUi.onPatchSuccess).catch(gameUi.failure);
	...
	var testGame = function testGame(event) {
...
/***/ function(module, exports) {
	...
	// used to store game response from server
...
/***/ function(module, exports, __webpack_require__) {
	...
	var onIndex = function onIndex(event) {
	  event.preventDefault();
	  gameApi.getIndex().then(gameUi.success).catch(gameUi.failure);
	};
	...
	var onCreate = function onCreate(event) {
	  ...
	  }).then(gameUi.createSuccess).catch(gameUi.failure);
	};

	var onShow = function onShow(event) {
	  event.preventDefault();
	  var id = parseInt($('#game-id').val());
	  gameApi.show(id).then(gameUi.success).catch(gameUi.failure);
	};

	var totalGamesPlayed = function totalGamesPlayed(event) {
	  event.preventDefault();
	  gameApi.getIndex().then(function (response) {
	  ...
	  }).then(gameUi.success).catch(gameUi.failure);
	};
